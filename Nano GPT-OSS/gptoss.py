...
@dataclass
class ModelConfig:
    num_hidden_layers: int = 12 # no of transformer blocks
    hidden_size: int = 2880 # size of token embedding
    head_dim: int = 64 # dimensionality of each attention head
    num_attention_heads: int = 64 # total number of query heads
    num_key_value_heads: int = 8 # no of key/value heads
    ...
    intermediate_size: int = 2880 #In the feed forward network, when network is expanded hidden_size
    swiglu_limit: float = 7.0


class RMSNorm(torch.nn.Module):
    def __init__(
        self, num_features: int, eps: float = 1e-5, device: torch.device | None = None
    ):
        super().__init__()
        self.num_features = num_features
        self.eps = eps
        self.scale = torch.nn.Parameter(torch.ones, device=device, dtype=torch.float32)
...
class RotaryEmbedding(torch.nn.Module):
    ...
    def _compute_concentration_and_inv_freq(self) -> torch.Tensor:
        freq = self.base ** ((torch.arange(0, self.head_dim, 2, dtype=torch.float, device=self.device))/ self.head_dim) #inverse theta
        if self.scaling_factor >1:
            concentration = (0.1* math.log(self.scaling_factor) + 1.0) #concentration factor
    ...
    def _compute_cos_sin(self, num_tokens: int):
        ...
        t = torch.arange(num_tokens, dtype=torch.float, device=self.device)
...
def sdpa(Q, K, V, S, sm_scale, sliding_window=0):
    ...
    #We are having GroupedQueryAttention + AttentionBias + SlidingWindowAttention
    #sm here mean softmax and sm_scale means softmax scaling
    ...
    #S initially has the single dimension with values n_heads*q_mult, one sink value query head
...
class MLPB(torch.nn.Module): #Multi-layer perceptron block
    def __init__(
        self,
        config: ModelConfig,
        device: torch.device | None = None,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Taking top-k experts
        experts = torch.topk(g, k=self.experts_per_token, dim=-1, sorted=True)
        expert_weights = torch.nn.functional.softmax(experts.values, dim=-1)
        expert_indices = experts.indices

        # flatenning the process
        ...
        for expert_idx in range(self.num_experts):
            ...
            # It is returning token indices which are going inside expert for each expert_idx

            expert_input = t_flat[token_indices]
            # normalized token embeddings which will go to expert in the for loop
            ...
            expert_out = self.experts[expert_idx][0](expert_out) # 1st linear + activation
...
class Transformer(torch.nn.Module):
    def __init__(
        ...
        self.embedding = torch.nn.Embedding(config.vocab_size, config.hidden_size,
                                            device=device, dytype=torch.bfloat16)
